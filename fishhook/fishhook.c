...
static void rebind_symbols_for_image(struct rebindings_entry *rebindings,
    ...
    for (uint32_t i = 0; i < header->ncmds; i++, cur += cur_seg_cmd->cmdsize) {
        ...
        if (strcmp(cur_seg_cmd->segname, SEG_DATA) &&
            strcmp(cur_seg_cmd->segname, SEG_DATA_CONST)) continue;
        section_t *sect = (section_t *)(cur_seg_cmd + 1);
        for (uint32_t j = 0; j < cur_seg_cmd->nsects; j++, sect++) {
